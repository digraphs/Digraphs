...
{
    DIGRAPHS_ASSERT(
        flags == EMBEDFLAGS_PLANAR || flags == EMBEDFLAGS_OUTERPLANAR ||
        flags == EMBEDFLAGS_SEARCHFORK23 || flags == EMBEDFLAGS_SEARCHFORK4 ||
        flags == EMBEDFLAGS_SEARCHFORK33);

    if (CALL_1ARGS(IsDigraph, digraph) != True) {
        ErrorQuit(
            "Digraphs: boyers_planarity_check (C): the 1st argument must be "
            "a digraph, not %s",
            (Int)TNAM_OBJ(digraph), 0L);
    }
    ...
    if (V > INT_MAX) {
        ...
        ErrorQuit(
            "Digraphs: boyers_planarity_check (C): the maximum number of "
            "nodes is %d, found %d",
            INT_MAX, V);
        return 0L;
    }
    ...
    if (gp_InitGraph(theGraph, V) != OK) {
        gp_Free(&theGraph);
        ErrorQuit(
            "Digraphs: boyers_planarity_check (C): invalid number of nodes!",
            0L, 0L);
        return 0L;
    }
        ...
        gp_Free(&theGraph);
        ErrorQuit(
            "Digraphs: boyers_planarity_check (C): invalid number of edges!",
            0L, 0L);
        return 0L;
    }
    ...
    for (Int v = 1; v <= LEN_LIST(out); ++v) {
        ...
        for (Int w = 1; w <= LEN_LIST(out_v); ++w) {
            ...
            if (v < u || CALL_3ARGS(IsDigraphEdge, digraph, INTOBJ_INT(u),
                                    INTOBJ_INT(v)) == False) {
    ...
    if (status == NOTOK) {
        ...
        gp_Free(&theGraph);
        ErrorQuit("Digraphs: boyers_planarity_check (C): status is not ok",
                  0L, 0L);
    }
    ...
    if (krtwsk) {
        ...
        for (int i = 1; i <= theGraph->N; ++i) {
            ...
            while (j) {
                if (CALL_3ARGS(IsDigraphEdge, digraph, INTOBJ_INT((Int)i),
