...
#A bipartite graph coming from a generalized quadrangle. So this graph must have diameter 4 and grith 8.
q := 8;
...
#another bipartite graph coming from a GQ, but now the GQ is constructed as a coset geometry.
#note that in this case the group used to construct the graph is much smaller than the complete automorphism group.
q := 3^3;
g := ElementaryAbelianGroup(q);
flist1 := [ Group(g.1), Group(g.2), Group(g.3), Group(g.1*g.2*g.3) ];;
flist2 := [ Group([g.1, g.2^2*g.3]), Group([g.2, g.1^2*g.3 ]), Group([g.3, g.1^2*g.2]), Group([g.1^2*g.2, g.1^2*g.3 ]) ];;
...
#A bipartite graph coming from a generalized hexagon. So this graph must have diameter 6 and grith 12.
q := 4;
