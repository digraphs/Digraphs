...
int RandomGraphs(char command, int NumGraphs, int SizeOfGraphs)
{
    ...
    int           writeResult;
    int           writeErrorReported_Random = FALSE,
        writeErrorReported_Embedded = FALSE,
        writeErrorReported_AdjList = FALSE,
        writeErrorReported_Obstructed = FALSE,
        writeErrorReported_Error = FALSE;

    GetNumberIfZero(&NumGraphs, "Enter number of graphs to generate:", 1,
                    1000000000);
    ...
    // Select a counter update frequency that updates more frequently with
    // larger graphs and which is relatively prime with 10 so that all digits
    // of the count will change even though we aren't showing the count value
    // on every iteration
    countUpdateFreq = 3579 / SizeOfGraphs;
    countUpdateFreq = countUpdateFreq < 1 ? 1 : countUpdateFreq;
    countUpdateFreq =
        countUpdateFreq % 2 == 0 ? countUpdateFreq + 1 : countUpdateFreq;
    countUpdateFreq =
    ...
    for (K = 0; K < NumGraphs; K++) {
        if ((Result = gp_CreateRandomGraph(theGraph)) == OK) {
            ...
            if (strchr("pdo234", command)) {
                ...
                if (gp_TestEmbedResultIntegrity(theGraph, origGraph,
                                                Result) != Result)
                    Result = NOTOK;
                ...
                if (Result == OK) {
                    ...
                    if (tolower(EmbeddableOut) == 'y') {
                        sprintf(theFileName, "embedded%c%d.txt",
                                FILE_DELIMITER, K % 10);
                        writeResult =
                            gp_Write(theGraph, theFileName, WRITE_ADJMATRIX);
                        if (writeResult != OK &&
                            !writeErrorReported_Embedded) {
                            sprintf(Line,
                                    "Failed to write graph %s\nMake the "
                    ...
                    if (tolower(AdjListsForEmbeddingsOut) == 'y') {
                        sprintf(theFileName, "adjlist%c%d.txt",
                                FILE_DELIMITER, K % 10);
                        writeResult =
                            gp_Write(theGraph, theFileName, WRITE_ADJLIST);
                        if (writeResult != OK &&
                            !writeErrorReported_AdjList) {
                            sprintf(Line,
                                    "Failed to write graph %s\nMake the "
                                    "directory if not present\n",
                                    theFileName);
                    ...
                    if (embedFlags == EMBEDFLAGS_PLANAR ||
                        ...
                        if (tolower(ObstructedOut) == 'y') {
                            sprintf(theFileName, "obstructed%c%d.txt",
                                    FILE_DELIMITER, K % 10);
                            writeResult = gp_Write(theGraph, theFileName,
                                                   WRITE_ADJMATRIX);
                            if (writeResult != OK &&
                                !writeErrorReported_Obstructed) {
                                sprintf(Line,
                                        "Failed to write graph %s\nMake the "
                                        "directory if not present\n",
                                        theFileName);
            ...
            // If there is an error in processing, then write the file for
            // debugging
    ...
    // Report statistics for subgraph homeomorphism algorithms
        ...
        sprintf(Line,
                "Of the generated graphs, %d did not contain a K_{2,3} "
        ...
        sprintf(Line,
                "Of the generated graphs, %d did not contain a K_{3,3} "
                "homeomorph as a subgraph.\n",
                MainStatistic);
        Message(Line);
    }
        ...
        sprintf(Line,
                "Of the generated graphs, %d did not contain a K_4 "
                "homeomorph as a subgraph.\n",
                MainStatistic);
        Message(Line);
    }
...
graphP MakeGraph(int Size, char command)
{
    ...
    // Enable the appropriate feature. Although the same code appears in
    // SpecificGraph, it is deliberately not separated to a common utility
    // because SpecificGraph is used as a self-contained tutorial.  It is not
    // that hard to update both locations when new algorithms are added.
                ...
                char * outfile2Name)
{
    ...
    platform_GetTime(start);
    if (gp_CreateRandomGraphEx(theGraph, 3 * numVertices - 6 + extraEdges) !=
        OK) {
    ...
    sprintf(Line, "Created random graph with %d edges in %.3lf seconds. ",
            theGraph->M, platform_GetDuration(start, end));
    Message(Line);
    FlushConsole(stdout);

    // The user may have requested a copy of the random graph before
    // processing
    ...
    if (strchr("pdo234", command)) {
        ...
        if (gp_TestEmbedResultIntegrity(theGraph, origGraph, Result) !=
            Result)
            Result = NOTOK;
    }
    else
    ...
    if (Result == OK || Result == NONEMBEDDABLE) {
        ...
        if (tolower(saveEdgeListFormat) == 'y') {
            ...
            if (extraEdges > 0)
                sprintf(theFileName, "random%cnonPlanarEdgeList.txt",
                        FILE_DELIMITER);
            else
                sprintf(theFileName, "random%cmaxPlanarEdgeList.txt",
                        FILE_DELIMITER);
