...
function(D)
  ...
  PostOrderFunc := function(record, data)
    ...
    # Backtracking on edge (current -> child), child is being backtracked on
    child := record.child;
    ...
    if current <> child then  # If not at the root node
      # Child could not reach any vertex visited before current
      ...
      # If a vertex can be visited from child that was visited before any
      # vertex that can be visited from current, the lowest reachable vertex
      # from current is updated
  ...
  AncestorCrossFunc := function(record, data)
    # current -> child is a back edge (cross edges were removed)
...
#   stack := Stack();
#   u := 1; # parent of node visiting
#   v := 1; # node visiting
#   i := 0;

#   repeat
#     if pre[v] <> -1 then
#       i := Pop(stack); # idx of successor being backtracked on
#       v := Pop(stack); # parent of successor (v visited from v -> nbs[v][i])
#       u := Pop(stack); # parent of parent of successor 

#       w := nbs[v][i]; # successor of v 

#       # backtracking w, on edge (v -> w)
...
#       low[v]  := counter;
#     fi;
#     i := PositionProperty(nbs[v], w -> w <> v, i);  # Looping through out neighbours of v
#     while i <> fail do
#       w := nbs[v][i]; # successor w of v
...
#       else # not a back edge, a tree edge
#         # diving - part 2
#         # edge traversal v -> w
#         if v = 1 then
...
#         Push(stack, u); # parent of v (parents parent)
#         Push(stack, v); # v - succssors of v being visited
#         Push(stack, i); # idx of successor w - continue visiting successors of v from this
#         u := v; # parent of w is v
#         v := w; # start backtracking from parent
#         i := 0; # reset idx of successors visiting
#         break;
#       fi;
#     od;
#   until Size(stack) = 0;
